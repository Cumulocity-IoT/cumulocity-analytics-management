...
- A fully automated (no manual re-select step) release-asset download-and-upload flow — blocked by the CORS constraint in NFR4 unless a proxy is introduced, which is out of scope for the zero-backend goal. **Under exploration, not yet in scope**: [DIRECT_UPLOAD.md](DIRECT_UPLOAD.md) proposes routing that proxy through `apama-ctrl` (already running per-tenant, so no *new* microservice, but a new responsibility for an existing one — a deliberate reversal of NFR1 this doc's own "Relationship to REQUIREMENTS.md" section flags for explicit sign-off, not a silent one). Status there is draft with several open risks/questions (R1–R3, Q1–Q6) and one working spike (`repository/epl/FetchExtensionListener.mon`, covering only the event-source lookup/bootstrap, not the fetch/upload itself) — this bullet stays accurate until that spec is actually approved.
