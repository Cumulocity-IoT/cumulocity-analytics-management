...
def get_repository_headers(request, repository_id: Optional[str] = None) -> Dict[str, str]:
...
def get_content():
    ...
    extract_fqn_cep_block = parse_boolean(request.args.get("extract_fqn_cep_block", False))
...
def update_repositories():
    ...
    if not isinstance(repositories, list):
        return create_error_response("Request body must be an array of repositories", 400)
    
    # Validate repository format
    required_fields = {"id", "name", "url"}
    for repo in repositories:
        if not all(field in repo for field in required_fields):
            return create_error_response("Each repository must have id, name, and url", 400)
        ...
        for monitor in monitors:
            try:
                ...
                repository_configuration = agent.load_repository(
                    request=request, 
                    repository_id=monitor["repositoryId"]
                )
...
def get_extension(name: str):
    ...
    Returns information about monitors included in the extension.
    """
...
def get_extension_metadata():
    ...
    Returns a list of all loaded extensions with their metadata.
    """
...
def get_cep_ctrl_status():
    """
    Get status of CEP ctrl microservice.
    """
...
class MonitorDownloader:
    ...
    @staticmethod
    async def download_monitor(url: str, headers: Dict[str, str], target_path: str) -> None:
